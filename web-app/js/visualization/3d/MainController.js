...
function loadLaserData() {
    var plane = new THREE.BoxGeometry(4, 4, 4);
    ...
    for (var i = 0; i < playerTree.length; i++) {
        if(!playerTree[i].destroyed && playerTree[i].model){
            ...
            if (!playerTree[i].data.linkdead) {
                ...
                var radian = Math.atan2(playerTree[i].data.lookingAt.y, playerTree[i].data.lookingAt.x);
                playerTree[i].model.rotation.y = radian;
